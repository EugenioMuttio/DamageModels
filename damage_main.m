...
% ----------------------------------
 LABELPLOT = {'hardening variable (q)','internal variable','C_{11} tangent','C_{11} algorithmic'};
...
vartoplot{i}(4) = ce(1,1); % Ce_tan11
vartoplot{i}(5) = ce(1,1); % Ce_alg11
...
for  iload = 1:length(istep)
    ...
    for iloc = 1:istep(iload)
        ...
        vartoplot{i}(2) = hvar_n(5) ; % Internal variable (r)        
        vartoplot{i}(3) = 1-hvar_n(6)/hvar_n(5)  ; %  Damage variable (d)
        vartoplot{i}(4) = aux_var(4); % Ce_tan11
        vartoplot{i}(5) = aux_var(5); % Ce_alg11
    end
end
